...
void ____utim_datatype_test____()
{
	 ...
	 */
	ASSERT_ON_BUILD(sizeof(utim_i32_t) - 4);
	/*
...
static UTIM_INLINE
utim_i32_t _index_calc(int xsize, int ysize, int x, int y)
{
	utim_i32_t index;
...
static utim_u8_t *_bmp_read(const char *filename,
	int *xsize, int *ysize, int *channels)
{
	utim_u8_t header[54];
	utim_u8_t color_dp;
	utim_u32_t pxl_begin, *x;
	utim_u32_t r, tmp, windows_byte_cnt = 0;
	utim_u8_t *image_tail, *pixels;
	FILE *fp;
	if (!(fp = fopen(filename, "rb"))) 
		return NULL;
	fread(header, sizeof(utim_u8_t), 54, fp);
	x = (utim_u32_t*)(header + 18);
	*xsize = x[0];
	*ysize = x[1];
	pxl_begin = *(utim_u32_t*)(header + 10);
#if   defined(RUN_TIME_ENDIAN_CHECK)
	if (!_is_little_endian) {
		_memrev(xsize, sizeof(utim_u32_t));
		_memrev(ysize, sizeof(utim_u32_t));
		_memrev(&pxl_begin, sizeof(utim_u32_t));
	}
	...
	pixels = (utim_u8_t*)malloc((*xsize) * (*ysize) * (*channels));
	if (!pixels) {
		fclose(fp);
		return NULL;
	}
	...
	for (r = 0; r < (*ysize); ++r) {
		...
		fread(image_tail , sizeof(utim_u8_t), (*xsize) * (*channels), fp);
...
static int _bmp_write(const char *filename,
	utim_u8_t *pixels, int xsize, int ysize, int channels)
{
	utim_u8_t header[54] = {
	...
	FILE *fp;
	utim_u8_t *image_tail;
	utim_u32_t windows_byte_cnt = 0;
	utim_u32_t windows_byte_set = 0;
	utim_u32_t tmp, r = (xsize * channels) % 4;
	utim_u32_t pxl_begin, *x;
	switch (channels) {
	case 1:
		pxl_begin = 54 + 1024;
	...
	x = (utim_u32_t*)(header + 2);
	x[0] = (xsize * channels + windows_byte_cnt) * ysize + 54;
	x = (utim_u32_t*)(header + 18);
	x[0] = xsize;
	x[1] = ysize;
	*(utim_u32_t*)(header + 10) = pxl_begin;
#if   defined(RUN_TIME_ENDIAN_CHECK)
	if (!_is_little_endian) {
		_memrev(x + 0, sizeof(utim_u32_t));
		_memrev(x + 1, sizeof(utim_u32_t));
		_memrev((utim_u32_t*)(header + 10),
			sizeof(utim_u32_t));  /* pxl_begin */
	}
#elif defined(BYTE_ORDER_BIG_ENDIAN)
	_memrev(x + 0, sizeof(utim_u32_t));
	_memrev(x + 1, sizeof(utim_u32_t));
	_memrev((utim_u32_t*)(header + 10),
		sizeof(utim_u32_t));  /* pxl_begin */
#endif
	*(utim_u8_t*)(header + 28) = channels << 3;
	if (!(fp = fopen(filename, "wb+")))
		return -1;
	fwrite(header, sizeof(utim_u8_t), 54, fp);
	image_tail = pixels + (ysize * xsize * channels);
	...
	for (r = 0; r < ysize; ++r) {
		image_tail -= (xsize * channels);
		fwrite(image_tail, sizeof(utim_u8_t) * channels, xsize, fp);
...
#endif
#ifdef USE3RD_STB_IMAGE
	comp = comp ? comp : img->channels;
	switch (*(utim_i32_t*)tbuf) {
	case UTIM_BMP:
		return stbi_write_bmp(filename, img->xsize,
			img->ysize, comp, img->pixels) - 1;
	case UTIM_PNG:
		quality = quality ? quality : png_default_compress;
		stbi_write_png_compression_level = quality;
		return stbi_write_png(filename, img->xsize,
			img->ysize, img->channels, img->pixels,
			img->xsize * img->channels) - 1;
	case UTIM_JPG:
		quality = quality ? quality : jpg_default_quality;
		return stbi_write_jpg(filename, img->xsize,
			img->ysize, comp, img->pixels, quality) - 1;
	case UTIM_TGA:
		return stbi_write_tga(filename, img->xsize,
			img->ysize, comp, img->pixels) - 1;
	default:
		return -1; /* Not supported file format */
	}
...
UTIM_IMG *utim_copy(UTIM_IMG *img)
{
	...
	cpy = (UTIM_IMG*)malloc(sizeof(UTIM_IMG));
	if (!cpy) {
		return NULL;
	}
	memcpy(cpy, img, sizeof(UTIM_IMG));
	cpy->pixels = (utim_u8_t*)malloc(size);
	if (!cpy->pixels) {
		free(cpy);
		return NULL;
	}
	memcpy(cpy->pixels, img->pixels, size);
	return cpy;
}

static utim_u8_t *_image_resize_nearest(UTIM_IMG *img, int x, int y)
{
	utim_u8_t *p;
	int i, j, ch_size = x * y;
	p = (utim_u8_t*)malloc(ch_size * img->channels);
	if (!p) {
		return NULL;
	}
	for (j = 0; j < y; ++j) {
...
#ifdef USE3RD_STB_IMAGE
static utim_u8_t *_image_resize_linear(UTIM_IMG *img, int x, int y)
{
	utim_u8_t *p;
	int ch_size = x * y;
	p = (utim_u8_t*)malloc(ch_size * img->channels);
...
UTIM_IMG *utim_rgb_from_rgba(UTIM_IMG *rgba)
{
	...
	rgb->pixels = (utim_u8_t*)malloc(ch_size * 3);
	if (!rgb->pixels) {
		free(rgb);
		return NULL;
	}
...
int utim_rgb2gray(UTIM_IMG *rgb)
{
	...
	new_pixels = (utim_u8_t*)malloc(ch_size);
	if (!new_pixels) {
		return UTIM_ERR_ALLOC;
	}
	p = new_pixels;
...
UTIM_IMG *utim_gray_from_rgb(UTIM_IMG *rgb)
{
	...
	ch_size = rgb->xsize * rgb->ysize;
	gray->pixels = (utim_u8_t*)malloc(ch_size);
	if (!gray->pixels) {
		free(gray);
		return NULL;
	}
...
int utim_gray2rgba(UTIM_IMG *gray)
{
	...
	ch_size = gray->xsize * gray->ysize;
	new_pixels = (utim_u8_t*)malloc(ch_size * 4);
	if (!new_pixels) {
		return UTIM_ERR_ALLOC;
	}
	for (i = 0; i < ch_size; ++i) {
...
UTIM_IMG *utim_create(int x, int y, int nch, int c)
{
	...
	img->pixels = (utim_u8_t*)malloc(size);
	if (!img->pixels) {
		free(img);
		return NULL;
	}
...
UTIM_IMG *utim_stack(UTIM_IMG **chx, int nch)
{
	...
	img->pixels = (utim_u8_t*)malloc(ch_size * nch);
	if (!img->pixels) {
		free(img);
		return NULL;
	}
...
UTIM_IMG *utim_pick_chl(UTIM_IMG *img, int ich)
{
	...
	ch_size = img->xsize * img->ysize;
	ch->pixels = (utim_u8_t*)malloc(ch_size);
	if (!ch->pixels) {
		free(ch);
		return NULL;
	}
	...
	UTIM_IMG *im, int i, int j, UTIM_POINT p)
{
	...
	utim_u8_t *rb = &c_bg[UTIM_COLOR_R];
	utim_u8_t *gb = &c_bg[UTIM_COLOR_G];
	utim_u8_t *bb = &c_bg[UTIM_COLOR_B];
	utim_u8_t *ab = &c_bg[UTIM_COLOR_A];
	utim_u8_t *ri = &c_im[UTIM_COLOR_R];
	utim_u8_t *gi = &c_im[UTIM_COLOR_G];
	utim_u8_t *bi = &c_im[UTIM_COLOR_B];
	utim_u8_t *ai = &c_im[UTIM_COLOR_A];
	...
	if (bg->channels == 4 && bg->channels == im->channels) { /* RGBA */
		...
		*ab = *ab + ((255 - *ab) * (*ai) / 255);
		...
		for (k = 0; k < bg->channels && k < im->channels; ++k) {
			bg->pixels[index_bg * bg->channels + k] =
			im->pixels[index_im * im->channels + k];
		}
	}
}
...
{
	UTIM_COLOR c_img;
	utim_u8_t *ri = &c[UTIM_COLOR_R];
	utim_u8_t *gi = &c[UTIM_COLOR_G];
	utim_u8_t *bi = &c[UTIM_COLOR_B];
	utim_u8_t *ai = &c[UTIM_COLOR_A];
	utim_u8_t *rb = &c_img[UTIM_COLOR_R];
	utim_u8_t *gb = &c_img[UTIM_COLOR_G];
	utim_u8_t *bb = &c_img[UTIM_COLOR_B];
	utim_u8_t *ab = &c_img[UTIM_COLOR_A];
	...
	if (img->channels == 4) { /* RGBA */
		...
		*ab = *ab + ((255 - *ab) * (*ai) / 255);
		...
		for (k = 0; k < img->channels; ++k) {
			img->pixels[index * img->channels + k] = c[k];
		}
	}
	return 0;
}
...
static unsigned int _read_utf8_value(utim_u8_t *character, int *nb)
{
	int i;
...
UTIM_FONT *utim_load_font(const char *filename)
{
	...
	font->glyphs = (utim_u8_t*)malloc(dsize);
	if (!font->glyphs) {
		free(font);
		fclose(fp);
		return NULL;
	...
	if (font->header.flags & PSF2_HAS_UNICODE_TABLE) {
		...
		utf_data = (utim_u8_t*)malloc(end - pos);
		if (!utf_data) {
			free(font->utf8index);
			free(font->glyphs);
			free(font);
...
static utim_u8_t *_get_character(UTIM_FONT *font, unsigned int number)
{
...
static utim_u8_t *_get_glyph(UTIM_FONT *font, char *character, int *nb)
{
	return _get_character(font, _glyph_index(
		font, _read_utf8_value((utim_u8_t*)character, nb)));
}
...
UTIM_IMG *utim_text(UTIM_FONT *font, char *text, UTIM_COLOR color)
{
	...
	for (i = 0; i < l; i = i + nb) {
		...
		charp = _get_glyph(font, text + i, &nb);
		if (charp) {
